...
#define CLASS_NAME                                                             \
  "charr" ///< The device class -- this is a character device drive
...
MODULE_DESCRIPTION(
    "charkmod_out Kernel Module"); ///< The description -- see modinfo
...
static struct class *charkmod_outClass =
    NULL; ///< The device-driver class struct pointer
static struct device *charkmod_outDevice =
...
int init_module(void) {
  ...
  if (IS_ERR(charkmod_outClass)) { // Check for error and clean up if there is
    unregister_chrdev(major_number, DEVICE_NAME);
    printk(KERN_ALERT "Failed to register device class\n");
    return PTR_ERR(
        charkmod_outClass); // Correct way to return an error on a pointer
  }
  printk(KERN_INFO "charkmod_out: device class registered correctly\n");

  // Register the device driver
  charkmod_outDevice = device_create(charkmod_outClass, NULL,
                                     MKDEV(major_number, 0), NULL, DEVICE_NAME);
  if (IS_ERR(charkmod_outDevice)) {   // Clean up if there is an error
    class_destroy(charkmod_outClass); // Repeated code but the alternative is
                                      // goto statements
    unregister_chrdev(major_number, DEVICE_NAME);
  ...
  printk(KERN_INFO
         "charkmod_out: device class created correctly\n"); // Made it! device
                                                            // was initialized
...
void cleanup_module(void) {
  ...
  device_destroy(charkmod_outClass,
                 MKDEV(major_number, 0));       // remove the device
...
static ssize_t read(struct file *filep, char *buffer, size_t len,
                    loff_t *offset) {
  printk(KERN_INFO "charkmod_out: g_buffer %d %d", g_buffer.start,
         g_buffer.end);
  ssize_t ind = 0;
  ...
  if (ind < len) {
    printk(KERN_INFO
           "charkmod_out: Buffer has %d bytes of content, requested %d.",
           ind, len);
  }
