...
#define CLASS_NAME                                                             \
  "charw" ///< The device class -- this is a character device drive
...
MODULE_DESCRIPTION(
    "charkmod_in Kernel Module"); ///< The description -- see modinfo
MODULE_VERSION("0.1");            ///< A version number to inform users

DEFINE_MUTEX(ebbchar_mutex); /// A macro that is used to declare a new mutex
                             /// that is visible in this file results in a
                             /// semaphore variable ebbchar_mutex with value 1
                             /// (unlocked) DEFINE_MUTEX_LOCKED() results in a
                             /// variable with value 0 (locked)
...
static struct class *charkmod_inClass =
    NULL; ///< The device-driver class struct pointer
static struct device *charkmod_inDevice =
...
int init_module(void) {
  ...
  if (IS_ERR(charkmod_inClass)) { // Check for error and clean up if there is
    unregister_chrdev(major_number, DEVICE_NAME);
    printk(KERN_ALERT "Failed to register device class\n");
    return PTR_ERR(
        charkmod_inClass); // Correct way to return an error on a pointer
  }
  printk(KERN_INFO "charkmod_in: device class registered correctly\n");

  // Register the device driver
  charkmod_inDevice = device_create(charkmod_inClass, NULL,
                                    MKDEV(major_number, 0), NULL, DEVICE_NAME);
  if (IS_ERR(charkmod_inDevice)) {   // Clean up if there is an error
    class_destroy(charkmod_inClass); // Repeated code but the alternative is
                                     // goto statements
    unregister_chrdev(major_number, DEVICE_NAME);
  ...
  printk(KERN_INFO
         "charkmod_in: device class created correctly\n"); // Made it! device
                                                           // was initialized

  // creates the buffer
  g_buffer = (word_buffer){.start = 0, .end = 0, .full = 0};
...
static int open(struct inode *inodep, struct file *filep) {
  int err;
  printk(KERN_INFO "charkmod_in: entered write.\n");
  err = mutex_lock_interruptible(&ebbchar_mutex);
  if (err) {
    printk(KERN_ALERT "charkmod_in: error acquiring lock\n");
  }
  printk(KERN_INFO "charkmod_in: acquired lock.\n");
  return 0;
}
...
static int close(struct inode *inodep, struct file *filep) {
  ...
  printk(KERN_INFO "charkmod_in: exiting write.\n");
  return 0;
}
...
static ssize_t write(struct file *filep, const char *buffer, size_t len,
  ...
  //         || (g_buffer.start <= g_buffer.end && BUFFER_SIZE + g_buffer.start
  //         - g_buffer.end < len)){
  //
  ...
  // no more writing
  if (g_buffer.full) {
    printk(KERN_INFO "charkmod_in: buffer is full. unable to write", 0);
    return 0;
  }

  for (i = 0; i < len; i++) {
    if ((g_buffer.end + 1) % BUFFER_SIZE ==
        g_buffer.start) { // when buffer is full, no more writing.
      break;
    }
  ...
  if ((g_buffer.end + 1) % BUFFER_SIZE ==
      g_buffer.start) { // this means we've filled it up
    g_buffer.full = 1;
  }
  if (i < len) {
    printk(KERN_INFO "charkmod_in: Buffer has %d bytes remaining, attempting "
                     "to write %d, truncating input",
           i, len);
  }
  printk(KERN_INFO "charkmod_in: wrote %d bytes to buffer", i);
  return i;
}
